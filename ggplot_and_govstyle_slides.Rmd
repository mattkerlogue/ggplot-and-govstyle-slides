...
.small[The fundamentals of a plot are a call to `ggplot()`, from the {ggplot2} pacakge.]
...
.small[To draw data you need to add (with a plus, like in maths) a `geom` (a geometric layer). A `geom` requires data (unless otherwise sepcified it inherits the data from the `ggplot()` call) and a mapping (which provides a set of aesthetics, through the `aes(...)` function)]
...
ggplot(gdat) +
  ...
  geom_text(aes(x = acronym, y = total_requests_received, 
                label = total_requests_received),
            size = 3, nudge_y = 45, colour = "gray40") +
  labs(title = "FOI Statistics Q2 2019",
       x = "Government Body",
       y = "Total Request Received") +
  theme_gov() +
